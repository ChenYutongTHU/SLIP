...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        # return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask, attention_probs_forward_hook=self.set_attn_probs,
        #                  attention_probs_backwards_hook=None)[0]
    def forward(self, x: torch.Tensor):
        if self.explainable:
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
            return x, self.attn_probs #attn_weights (to-improve)
        else:
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, 
        ...
        self.layers = layers
        self.explainable = explainable
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, explainable=explainable) for _ in range(layers)])
...
def convert_weights(model: nn.Module, explainable=False):
    ...
    def _convert_weights_to_fp16(l):
        ...
        if explainable and isinstance(l, MultiheadAttention):
            for attr in [*[f"{s}_proj_weight" for s in ["in", "q", "k", "v"]], "in_proj_bias", "bias_k", "bias_v"]:
                tensor = getattr(l, attr)
                if tensor is not None:
                    tensor.data = tensor.data.half()
        if not explainable and isinstance(l, nn.MultiheadAttention):
            for attr in [*[f"{s}_proj_weight" for s in ["in", "q", "k", "v"]], "in_proj_bias", "bias_k", "bias_v"]:
                tensor = getattr(l, attr)
                if tensor is not None:
                    tensor.data = tensor.data.half()            
...
def build_model(state_dict: dict, return_full_embed = False, explainable=False):
    ...
    convert_weights(model, explainable=explainable)
