...
args.input = 'img2poem/id2poem_57.json'
...
files = [int(f.split('_')[0].split('.')[-1]) for f in os.listdir(os.path.dirname(args.output)) if 'chatgpt.json' in f and str(save_total) in f]
save_cnt = max(files)+1
...
with Pool(args.process_number) as p:
    ...
    for i, result in tqdm(p.imap_unordered(process_func, remain_ids), total=len(remain_ids)):
        print(i, id2poem[i], result)
        id2results[i] = result
        #if len(id2results)%args.save_frequency==0:
        if len(id2results)%200==0:
