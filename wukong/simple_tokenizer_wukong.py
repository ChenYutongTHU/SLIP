...
def tokenize(texts: Union[str, List[str]], padding='max') -> np.ndarray:
    """
    ...
    assert padding in ['max', 'longest']
    if padding == 'max':
        pad_len = CONTEXT_LEN
    elif padding == 'longest':
        pad_len = max([len(a) for a in all_tokens])
    else:
        raise ValueError
    result = np.zeros((len(all_tokens), pad_len), dtype=np.int64)
